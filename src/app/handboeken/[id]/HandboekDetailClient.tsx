...
interface HandboekDetailClientProps {
  initialHandboek: Handboek | null;
  initialHoofdstukken: Hoofdstuk[];
  initialAfbeeldingen: Record<string, Afbeelding[]>;
  handboekId: string;
  initialError: string | null;
}

export default function HandboekDetailClient({
  initialHandboek,
  initialHoofdstukken,
  initialAfbeeldingen,
  handboekId,
  initialError,
}: HandboekDetailClientProps) {
  const { user, isLoading: authLoading } = useAuth();

  const [handboek, setHandboek] = useState<Handboek | null>(initialHandboek);
  const [hoofdstukken, setHoofdstukken] = useState<Hoofdstuk[]>(initialHoofdstukken);
  const [afbeeldingenPerHoofdstuk, setAfbeeldingenPerHoofdstuk] = useState<Record<string, Afbeelding[]>>(initialAfbeeldingen);
  ...
  useEffect(() => {
    // Als we initialError hebben, skip fetch
    if (!user || initialError || !handboekId) return;
    if (handboek && hoofdstukken.length > 0) return; // al gevuld
    ...
    const fetchData = async () => {
      ...
      if (hoofdstukkenError) {
        setError('Kon hoofdstukken niet laden');
      } else {
    ...
    fetchData();
  }, [user, handboekId, initialError, handboek, hoofdstukken.length]);
  ...
  const handleDeleteHandboek = async () => {
    ...
    if (error) {
      ...
      window.location.href = '/handboeken';
    }
  };
